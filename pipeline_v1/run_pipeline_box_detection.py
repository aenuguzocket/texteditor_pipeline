"""
Background Box Detection Pipeline
=================================
...
def detect_boxes_in_layer(layer_path: str, text_regions: List[Dict], 
    ...
    for label_id in range(1, num_labels):  # Skip 0 (background)
        ...
        for region in text_regions:
            # FILTER: Only include CTA roles for box selection
            role = region.get("gemini_analysis", {}).get("role", "body")
            if role != "cta":
                continue  # Skip non-CTA regions
        ...
        if original_layer_name and boxes:
            ...
            if original_layer_path.exists():
                ...
                for idx, box in enumerate(boxes):
                    # Get box coordinates in LAYER space (not original)
...
if __name__ == "__main__":
    ...
    RUN_DIR = "pipeline_outputs/run_1767781547_layered"
