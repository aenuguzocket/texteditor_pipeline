    ...
    def _merge_close_regions(self, polys: List[np.ndarray]) -> List[np.ndarray]:
        ...
        for line in lines:
            ...
            for r in line[1:]:
                ...
                # Height Similarity Check (Prevent merging huge headings with small logos)
                # If one is significantly taller than the other (e.g. > 1.5x), don't merge
                h_ratio = max(prev["height"], r["height"]) / min(prev["height"], r["height"]) if min(prev["height"], r["height"]) > 0 else 999
                
                if gap < (1.5 * max_h) and h_ratio < 1.5: 
                    # Merge them
